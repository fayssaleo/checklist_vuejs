...
export default {
  ...
  methods: {
    ...
    changeEquipmentsFiltreSELECT() {
      ...
      this.getEquipmentDamagesMergedWithDamageTypesAction(
        ...
        this.DamagesMergedWithDamageTypes.map((item) => {
          //  debugger;
          if (item.department.name == TEC && item.damage != null) {
            if (item.damage.status == "closed") {
              this.damageTypesTEC.push(item);
            } else {
              this.modelDamageTEC.push(item);
            }
          } else if (item.department.name == TEC && item.damage == null) {
            this.damageTypesTEC.push(item);
          }

          if (item.department.name == IT && item.damage != null) {
            if (item.damage.status == "closed") {
              this.damageTypesIT.push(item);
            } else {
              this.modelDamageIT.push(item);
            }
          } else if (item.department.name == IT && item.damage == null) {
            this.damageTypesIT.push(item);
          }
        });
      });
    ...
    valider(item) {
      ...
      Damage.declaredBy_id = parseInt(localStorage.getItem("userid") + "");
